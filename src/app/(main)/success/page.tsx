...
export default async function SuccessPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
    ...
    return (
        <div className={styles.paymentStatus}>
            <div className={styles.statusIcon} style={{ backgroundColor: statusContent.iconColor }}>
                {statusContent.icon}
            </div>
            <h2>{statusContent.text}</h2>
            <div className={styles.detailsTable}>
                <table>
                    <tbody>
                        <tr>
                            <td className={styles.tableLabel}>id</td>
                            <td className={styles.tableContent}>{paymentIntentId}</td>
                        </tr>
                        <tr>
